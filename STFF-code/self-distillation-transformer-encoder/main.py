...
def get_args_parser():
    ...
    # Misc
    parser.add_argument('--data_path', default='Z:\gf-3-nolabel/', type=str,
        help='Please specify path to the ImageNet training data.')
    parser.add_argument('--output_dir', default="Z:\STFF-code/pth/", type=str,
    ...
    for it, (images, labels, path, masks) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            for superpixel in range(len(path)):
                ...
                aquaculture_path = os.path.join('Z:\gf-3-nolabel/aquaculture/', file_name)
            ...
            for superpixel in range(len(path)):
                ...
                if os.path.exists(aquaculture_path):
                    ...
                    os.makedirs("Z:/STFF-code/feature_edge_map", exist_ok=True)
                    fname = os.path.join("Z:/STFF-code/feature_edge_map", "attn-head" + str(2) + ".png")
                    plt.imsave(fname=fname, arr=teacher_edge_output[2], format='png')
                    feature_edge_map = cv2.imread(fname, 0)

                    ret, thresh = cv2.threshold(feature_edge_map, 127, 255, cv2.THRESH_TRUNC+cv2.THRESH_OTSU)
